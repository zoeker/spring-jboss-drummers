...
public class MapUtilizer {
    ...
    public static <T> Map<T, Integer> sortByClassAndValue(
            final Map<T, Integer> map) {
        final List<Map.Entry<T, Integer>> list = new LinkedList<Map.Entry<T, Integer>>(
                map.entrySet());

        Collections.sort(list, new Comparator<Map.Entry<T, Integer>>() {

            @Override
            public int compare(final Map.Entry<T, Integer> m1,
                    final Map.Entry<T, Integer> m2) {
    ...
    public static Map<String, Integer> sortByValue(
        ...
        final List<Map.Entry<String, Integer>> list = new LinkedList<Map.Entry<String, Integer>>(
                map.entrySet());

        Collections.sort(list, new Comparator<Map.Entry<String, Integer>>() {

            @Override
            public int compare(final Map.Entry<String, Integer> m1,
                    final Map.Entry<String, Integer> m2) {
                return (m2.getValue()).compareTo(m1.getValue());
            }
        });
