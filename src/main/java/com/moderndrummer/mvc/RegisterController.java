...
public class RegisterController {
  ...
  public String registerNewMember(@Valid @ModelAttribute("newMember") RegisterFormModel registerModel, BindingResult result, Model model, HttpServletRequest request )
  {
      if (!result.hasErrors()) {
          try{
            ...
            Member member = userService.registerMember(newMember);
  ...
  private void validateMember(Member newMember, RegisterFormModel model) throws ModernDrummerException {
    if(!model.getPassword().equals(model.getConfirmPassword())){
      throw new ModernDrummerException(DrumChopsMessages.PASSWORD_NOT_EQUAL);
    }
    if(!model.getEmail().equals(model.getConfirmEmail())){
      throw new ModernDrummerException(DrumChopsMessages.EMAIL_NOT_EQUAL);
    }
    updateMember(newMember, model);
    if(!memberValidator.isInsertableMember(newMember)){
      throw new ModernDrummerException(DrumChopsMessages.FIELDS_INVALID);
    }
    Member foundMember = userService.findMemberByUserNameOrEmail(newMember.getName(), newMember.getEmail());
    if(foundMember.getId() > 0){
      throw new ModernDrummerException(DrumChopsMessages.FIELDS_EXISTS + " " + newMember.getName() + " " + newMember.getEmail());
    }
  }
